    ...
    def append(self, d):
        ...
        if self.is_train:
            train_anchors_, train_anchor_inds_, train_anchor_targets_, train_anchor_labels_ = \
                anchor_target_layer(gt_boxes_, (h, w), dataset='gt')
...
class GTBlob(object):
    ...
    def append(self, d):
        ...
        self.gt_aff_classes.append(np.column_stack((
            i * np.ones(d['gt_aff_classes'].shape[0], dtype=np.int64),
            d['gt_aff_classes'],
        )))

        self.gt_att_classes.append(np.column_stack((
            i * np.ones(d['gt_att_classes'].shape[0], dtype=np.int64),
            d['gt_att_classes'],
        )))
    ...
    def reduce(self):
        ...
        self.gt_boxes, self.gt_box_chunks = self._chunkize(self.gt_boxes, tensor=torch.FloatTensor)
        self.gt_classes, _ = self._chunkize(self.gt_classes)
        self.gt_aff_classes, _ = self._chunkize(self.gt_aff_classes)
        self.gt_att_classes, _ = self._chunkize(self.gt_att_classes)
    ...
    def scatter(self):
        ...
        # Predcls might need these
        self.gt_classes = self._scatter(self.gt_classes, self.gt_box_chunks)
        self.gt_aff_classes = self._scatter(self.gt_aff_classes, self.gt_box_chunks)
        self.gt_att_classes = self._scatter(self.gt_att_classes, self.gt_box_chunks)
        self.gt_boxes = self._scatter(self.gt_boxes, self.gt_box_chunks)
    ...
    def __getitem__(self, index):
        ...
        if index == 0 and self.num_gpus == 1:
            image_offset = 0
            if self.is_train:
                return (self.imgs, self.im_sizes[0], image_offset, self.gt_boxes,
                        self.gt_classes, self.gt_aff_classes, self.gt_att_classes,
                        rels, proposals, self.train_anchor_inds)
            return self.imgs, self.im_sizes[0], image_offset, self.gt_boxes, \
                   self.gt_classes, self.gt_aff_classes, self.gt_att_classes, rels, proposals
        ...
        if self.is_train:
            return (
            self.imgs[index], self.im_sizes[index], image_offset,
            self.gt_boxes[index], self.gt_classes[index], self.gt_aff_classes[index], self.gt_att_classes[index],
            rels_i, None, self.train_anchor_inds[index])
        return (self.imgs[index], self.im_sizes[index], image_offset,
                self.gt_boxes[index], self.gt_classes[index], self.gt_aff_classes[index], self.gt_att_classes[index],
                rels_i, None)