...
from dataloaders.blob import GTBlob, Blob
from lib.fpn.box_intersections_cpu.bbox import bbox_overlaps
from config import GT_IMAGES, IM_DATA_FN_GT, GT_SGG_FN, GT_SGG_DICT_FN, BOX_SCALE_GT, IM_SCALE_GT, PROPOSAL_FN_GT
...
class GT(Dataset):
    def __init__(self, mode, roidb_file=GT_SGG_FN, dict_file=GT_SGG_DICT_FN,
                 image_file=IM_DATA_FN_GT, filter_empty_rels=True, num_im=-1, num_val_im=5000,
                 filter_duplicate_rels=True, filter_non_overlap=False, use_proposals=False):
        """
        ...
        self.split_mask, self.gt_boxes, self.gt_classes, self.gt_aff_classes, self.gt_att_classes, self.relationships = \
            load_graphs(self.roidb_file, self.mode, num_im, num_val_im=num_val_im,
            filter_empty_rels=filter_empty_rels, filter_non_overlap=self.filter_non_overlap and self.is_train,
        )

        self.filenames = load_image_filenames(image_file)
        self.filenames = [self.filenames[i] for i in np.where(self.split_mask)[0]]

        self.ind_to_classes, self.ind_to_aff_classes, self.ind_to_att_classes, self.ind_to_predicates = \
            load_info(dict_file)

        if use_proposals:
            print("Loading proposals", flush=True)
            p_h5 = h5py.File(PROPOSAL_FN_GT, 'r')
        ...
        tform = [
            ...
            Resize(IM_SCALE_GT),
            ToTensor(),
            Normalize(mean=[0.249, 0.249, 0.249], std=[0.432, 0.433, 0.432]),
        ]
    ...
    def __getitem__(self, index):
        ...
        # Boxes are already at BOX_SCALE_GT
        if self.is_train:
            ...
            gt_boxes[:, [1, 3]] = gt_boxes[:, [1, 3]].clip(
                None, BOX_SCALE_GT / max(image_unpadded.size) * image_unpadded.size[1])
            gt_boxes[:, [0, 2]] = gt_boxes[:, [0, 2]].clip(
                None, BOX_SCALE_GT / max(image_unpadded.size) * image_unpadded.size[0])

            # # crop the image for data augmentation
            # image_unpadded, gt_boxes = random_crop(image_unpadded, gt_boxes, BOX_SCALE_GT, round_boxes=True)

        w, h = image_unpadded.size
        box_scale_factor = BOX_SCALE_GT / max(w, h)
        ...
        img_scale_factor = IM_SCALE_GT / max(w, h)
        if h > w:
            im_size = (IM_SCALE_GT, int(w * img_scale_factor), img_scale_factor)
        elif h < w:
            im_size = (int(h * img_scale_factor), IM_SCALE_GT, img_scale_factor)
        else:
            im_size = (IM_SCALE_GT, IM_SCALE_GT, img_scale_factor)
        ...
        if self.filter_duplicate_rels:
            ...
            for (o0, o1, r) in zip(gt_rels[:, 0], gt_rels[:, 1], gt_rels[:, 2:]):
        ...
        entry = {
            ...
            'gt_aff_classes': self.gt_aff_classes[index][:, 1:].copy(),
            'gt_att_classes': self.gt_att_classes[index][:, 1:].copy(),
            'gt_relations': gt_rels,
            'scale': IM_SCALE_GT / BOX_SCALE_GT,  # Multiply the boxes by this.
            'index': index,
            'flipped': flipped,
...
def assertion_checks(entry):
    ...
    assert entry['gt_relations'].shape[0] > 0


def load_image_filenames(image_file, image_dir=GT_IMAGES):
    """
...
def load_graphs(graphs_file, mode='train', num_im=-1, num_val_im=0, filter_empty_rels=True,
    ...
    # Get box information
    category_labels = roi_h5['category_labels'][:]
    affordance_labels = roi_h5['affordance_labels'][:]
    attribute_labels = roi_h5['attribute_labels'][:]
    all_boxes = roi_h5['boxes_{}'.format(BOX_SCALE_GT)][:]  # will index later
    ...
    _relation_predicates = roi_h5['predicates'][:]
    ...
    for i in range(len(image_index)):
        ...
        gt_classes_i = category_labels[im_to_first_box[i]:im_to_last_box[i] + 1]
        gt_aff_classes_i = affordance_labels[im_to_first_box[i]:im_to_last_box[i] + 1]
        gt_att_classes_i = attribute_labels[im_to_first_box[i]:im_to_last_box[i] + 1]

        if im_to_first_rel[i] >= 0:
            predicates = _relation_predicates[im_to_first_rel[i]:im_to_last_rel[i] + 1]
            obj_idx = _relations[im_to_first_rel[i]:im_to_last_rel[i] + 1] - im_to_first_box[i] # convert global index to local index
            assert np.all(obj_idx >= 0)
            assert np.all(obj_idx < boxes_i.shape[0])
            # remove bg class of the relation prediction
            rels = np.column_stack((obj_idx, predicates[:, 1:]))
        else:
        ...
        assert rels.shape[1] == 6
        assert rels.shape[0] != 0

        boxes.append(boxes_i)
        gt_classes.append(gt_classes_i)
        gt_aff_classes.append(gt_aff_classes_i)
        gt_att_classes.append(gt_att_classes_i)
        relationships.append(rels)

    return split_mask, boxes, gt_classes, gt_aff_classes, gt_att_classes, relationships
...
def load_info(info_file):
    ...
    # temporarily remove it since we've included the None class
    # info['label_to_idx']['__background__'] = 0
    # info['predicate_to_idx']['__background__'] = 0

    class_to_ind = info['category_label_to_idx']
    aff_class_to_ind = info['affordance_label_to_idx']
    att_class_to_ind = info['attribute_label_to_idx']
    predicate_to_ind = info['predicate_to_idx']
    ind_to_classes = sorted(class_to_ind, key=lambda k: class_to_ind[k])
    ind_to_aff_classes = sorted(aff_class_to_ind, key=lambda k: aff_class_to_ind[k])
    ind_to_att_classes = sorted(att_class_to_ind, key=lambda k: att_class_to_ind[k])
    ind_to_predicates = sorted(predicate_to_ind, key=lambda k: predicate_to_ind[k])

    return ind_to_classes, ind_to_aff_classes, ind_to_att_classes, ind_to_predicates


def vg_collate(data, num_gpus=3, is_train=False, mode='det'):
    assert mode in ('det', 'rel')
    if mode == 'rel':
        blob = GTBlob(mode=mode, is_train=is_train, num_gpus=num_gpus,
