...
if conf.coco:
    ...
    train_loader, val_loader = GTDataLoader.splits(train, val, batch_size=conf.batch_size,
                                                   num_workers=conf.num_workers,
                                                   num_gpus=conf.num_gpus)
...
for epoch in range(start_epoch + 1, start_epoch + 1 + conf.num_epochs):
    ...
    }, os.path.join(conf.save_dir, '{}-{}.tar'.format('coco' if conf.coco else 'gt', epoch)))

    torch.save({
        'epoch': epoch,
        'state_dict': detector.features.state_dict(),
        'optimizer': optimizer.state_dict(),
    }, os.path.join(conf.save_dir, '{}-{}.tar'.format('coco' if conf.coco else 'gt_resnet_backbone', epoch)))
