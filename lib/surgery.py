...
def filter_dets(boxes, obj_scores, obj_classes, rel_inds, pred_scores):
    """
    Filters detections (actually doesn't filter anything but sort the relation prediction based on relation confidence
    score, confidence scores of object prediction in bbx1 and bbx2.)
...
def filter_dets_gt(boxes, obj_scores, obj_classes, obj_aff_scores, obj_aff_classes,
                   obj_att_scores, obj_att_classes, rel_inds, pred_rel_scores):
    """
    Filters detections (actually doesn't filter anything but sort the relation prediction based on relation confidence
    score, confidence scores of object prediction in bbx1 and bbx2, confidence scores of affordance prediction
    in bbx1 and bbx2, confidence scores of attribute prediction in bbx1 and bbx2.)
    :param boxes: [num_box, topk, 4] if bbox regression else [num_box, 4]
    :param obj_scores: [num_box] the highest object probability for each bbx
    :param obj_classes: [num_box] class index for object prediction in each bbx
    :param obj_aff_scores: [num_bbx] the highest affordance probability for each bbx
    :param obj_aff_classes: [num_box] class index for affordance detection in each bbx
    :param obj_att_scores: [num_bbx] the highest attribute probability for each bbx
    :param obj_att_classes: [num_box] class index for attribute detection in each bbx
    :param rel_inds: [num_rel, 2] TENSOR consisting of (im_ind0, im_ind1)
    :param pred_rel_scores: [topk, topk, num_rel, num_predicates]
    :param use_nms: True if use NMS to filter dets.
    :return: boxes, objs, rels, pred_scores
    ...
    num_rel = rel_inds.size(0)
    assert rel_inds.size(1) == 2
    assert pred_rel_scores.size(0) == num_rel

    obj_scores0 = obj_scores.data[rel_inds[:,0]]
    obj_scores1 = obj_scores.data[rel_inds[:,1]]

    obj_aff_scores0 = obj_aff_scores.data[rel_inds[:, 0]]
    obj_aff_scores1 = obj_aff_scores.data[rel_inds[:, 1]]

    obj_att_scores0 = obj_att_scores.data[rel_inds[:, 0]]
    obj_att_scores1 = obj_att_scores.data[rel_inds[:, 1]]

    rel_index = (pred_rel_scores > 0.5).nonzero(as_tuple=False)
    pred_scores_max = torch.ones_like(obj_scores0)
    for index, label in rel_index:
        pred_scores_max[index] *= pred_rel_scores[index, label]

    rel_scores_argmaxed = pred_scores_max * obj_scores0 * obj_scores1 * obj_aff_scores0 * \
                          obj_aff_scores1 * obj_att_scores0 * obj_att_scores1
    # rel_scores_argmaxed = pred_scores_max * obj_scores0 * obj_scores1

    rel_scores_vs, rel_scores_idx = torch.sort(rel_scores_argmaxed.view(-1), dim=0, descending=True)

    rels = rel_inds[rel_scores_idx].cpu().numpy()
    # pred_rel_scores = (pred_rel_scores > 0.5).float()
    pred_scores_sorted = pred_rel_scores[rel_scores_idx].data.cpu().numpy()
    obj_scores_np = obj_scores.data.cpu().numpy()
    objs_np = obj_classes.data.cpu().numpy()
    obj_aff_scores_np = obj_aff_scores.data.cpu().numpy()
    objs_aff_np = obj_aff_classes.data.cpu().numpy()
    obj_att_scores_np = obj_att_scores.data.cpu().numpy()
    objs_att_np = obj_att_classes.data.cpu().numpy()
    boxes_out = boxes.data.cpu().numpy()

    return boxes_out, objs_np, obj_scores_np, objs_aff_np, obj_aff_scores_np, \
           objs_att_np, obj_att_scores_np, rels, pred_scores_sorted
