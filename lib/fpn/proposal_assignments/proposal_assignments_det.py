...
@to_variable
def proposal_assignments_det_3labels(rpn_rois, gt_boxes, gt_classes, gt_aff_classes, gt_att_classes,
    ...
    for im_ind in range(num_images):
        ...
        labels_ = gt_classes[:, 1][gt_assignment[keep_inds]]
        aff_labels_ = gt_aff_classes[:, 1:][gt_assignment[keep_inds]]
        att_labels_ = gt_att_classes[:, 1:][gt_assignment[keep_inds]]
        ...
        if num_fg < labels_.size(0):
            labels_[num_fg:] = 0
            # ToDo: if it is necessary to do the same thing for affordance and attribute classes
        ...
        labels.append(labels_)
        aff_labels.append(aff_labels_)
        att_labels.append(att_labels_)
        rois.append(rois_)
        bbox_targets.append(bbox_target_)

    rois = torch.cat(rois, 0)
    labels = torch.cat(labels, 0)
    aff_labels = torch.cat(aff_labels, 0)
    att_labels = torch.cat(att_labels, 0)
    bbox_targets = torch.cat(bbox_targets, 0)
    return rois, labels, aff_labels, att_labels, bbox_targets
