...
def to_multihot(vec):
    '''
    :param vec: ground-truth vector of multi-label classification, e.g., [1, 0, 0, 1, 0]
    :return: a vector with the same size and assigning 1 to 1 and -1 to 0
    '''
    return (vec * 2 - 1).float()
...
def transpose_packed_sequence_inds(lengths):
    ...
    # return new_inds, torch.tensor(new_lens)
