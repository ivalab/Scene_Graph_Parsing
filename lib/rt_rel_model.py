...
from lib.lstm.decoder_rnn import DecoderRNN, MultiLabelDecoderRNN
# from lib.lstm.highway_lstm_cuda.alternating_highway_lstm import AlternatingHighwayLSTM
from lib.stacked_alternating_lstm import StackedAlternatingLstm
from lib.fpn.box_utils import bbox_overlaps, center_size
from lib.get_union_boxes import UnionBoxesAndFeats
from lib.fpn.proposal_assignments.rel_assignments import rel_assignments
from lib.gt_object_detector import GTObjectDetector, gather_res, load_vgg
from lib.pytorch_misc import transpose_packed_sequence_inds, to_onehot, to_multihot, \
    arange, enumerate_by_image, diagonal_inds, Flattener
# from lib.sparse_targets import FrequencyBias
from lib.surgery import filter_dets, filter_dets_gt
...
def _sort_by_score(im_inds, scores):
    ...
    rois_per_image = scores.new_zeros((num_im))
    lengths = []
...
class LinearizedContext(nn.Module):
    ...
    """
    def __init__(self, classes, aff_classes, att_classes, rel_classes, mode='sgdet',
        ...
        # class for visual entity
        self.classes = classes
        self.aff_classes = aff_classes
        self.att_classes = att_classes

        # class for relation between visual entities
        self.rel_classes = rel_classes

        # manually define number of classes
        self.num_classes = len(self.classes)
        self.num_aff_classes = len(self.aff_classes)
        ...
        # EMBEDDINGS (object category, affordance, attribute)
        obj_cate_embed_vecs = obj_edge_vectors(self.classes, wv_dim=self.embed_dim)
        obj_aff_embed_vecs = obj_edge_vectors(self.aff_classes, wv_dim=self.embed_dim)
        obj_att_embed_vecs = obj_edge_vectors(self.att_classes, wv_dim=self.embed_dim)

        self.obj_cate_embed = nn.Embedding(self.num_classes, self.embed_dim)
        self.obj_cate_embed.weight.data = obj_cate_embed_vecs.clone()
        # self.obj_aff_embed = nn.Embedding(self.num_aff_classes, self.embed_dim)
        # self.obj_aff_embed.weight.data = obj_aff_embed_vecs.clone()
        # self.obj_att_embed = nn.Embedding(self.num_att_classes, self.embed_dim)
        # self.obj_att_embed.weight.data = obj_att_embed_vecs.vecs.clone()

        self.obj_cate_embed2 = nn.Embedding(self.num_classes, self.embed_dim)
        self.obj_cate_embed2.weight.data = obj_cate_embed_vecs.clone()
        self.obj_aff_embed2 = nn.Embedding(self.num_aff_classes, self.embed_dim)
        self.obj_aff_embed2.weight.data = obj_aff_embed_vecs.clone()
        self.obj_att_embed2 = nn.Embedding(self.num_att_classes, self.embed_dim)
        self.obj_att_embed2.weight.data = obj_att_embed_vecs.clone()

        self.m_act = nn.Sigmoid()
        ...
        if self.nl_obj > 0:
            self.obj_ctx_rnn = StackedAlternatingLstm(
            ...
            # decoder for object category
            self.decoder_rnn = DecoderRNN(self.classes, embed_dim=self.embed_dim,
                                          inputs_dim=decoder_inputs_dim,
                                          hidden_dim=self.hidden_dim,
                                          recurrent_dropout_probability=dropout_rate)
            # decoder for object affordance
            self.aff_decoder_rnn = MultiLabelDecoderRNN(self.aff_classes, embed_dim=self.embed_dim,
                                              inputs_dim=decoder_inputs_dim,
                                              hidden_dim=self.hidden_dim,
                                              recurrent_dropout_probability=dropout_rate)

            # decoder for object attribute
            self.att_decoder_rnn = MultiLabelDecoderRNN(self.att_classes, embed_dim=self.embed_dim,
                                              inputs_dim=decoder_inputs_dim,
                                              hidden_dim=self.hidden_dim,
                                              recurrent_dropout_probability=dropout_rate)
        else:
        ...
        if self.nl_edge > 0:
            # times 3 since there are category, affordance and attribute
            ...
            self.edge_ctx_rnn = StackedAlternatingLstm(input_size=input_dim,
                                                       hidden_size=self.hidden_dim,
    ...
    def edge_ctx(self, obj_feats, obj_dists, obj_aff_dists, obj_att_dists, im_inds,
                 obj_preds, obj_aff_preds, obj_att_preds, box_priors=None):
        """
        ...
        obj_embed2 = self.obj_cate_embed2(obj_preds)
        # fuse multi-label affordance embeddings
        obj_aff_embed = None
        for pred_i in obj_aff_preds:
            bbx_embed = None
            for label_idx, pred_value in enumerate(pred_i):
                if pred_value == 1:
                    if bbx_embed is None:
                        bbx_embed = self.obj_aff_embed2(torch.tensor(label_idx, dtype=torch.int64, device='cuda'))
                    else:
                        bbx_embed += self.obj_aff_embed2(torch.tensor(label_idx, dtype=torch.int64, device='cuda'))

            # if nothing is predicted, consider the detection as background
            if bbx_embed is None:
                bbx_embed = self.obj_aff_embed2(torch.tensor(0, dtype=torch.int64, device='cuda'))
            bbx_embed = bbx_embed.unsqueeze(0)

            if obj_aff_embed is None:
                obj_aff_embed = bbx_embed
            else:
                obj_aff_embed = torch.cat((obj_aff_embed, bbx_embed), 0)

        # fuse multi-label attribute embeddings
        obj_att_embed = None
        for pred_i in obj_att_preds:
            bbx_embed = None
            for label_idx, pred_value in enumerate(pred_i):
                if pred_value == 1:
                    if bbx_embed is None:
                        bbx_embed = self.obj_att_embed2(torch.tensor(label_idx, dtype=torch.int64, device='cuda'))
                    else:
                        bbx_embed += self.obj_att_embed2(torch.tensor(label_idx, dtype=torch.int64, device='cuda'))

            # if nothing is predicted, consider the detection as background
            if bbx_embed is None:
                bbx_embed = self.obj_att_embed2(torch.tensor(0, dtype=torch.int64, device='cuda'))
            bbx_embed = bbx_embed.unsqueeze(0)

            if obj_att_embed is None:
                obj_att_embed = bbx_embed
            else:
                obj_att_embed = torch.cat((obj_att_embed, bbx_embed), 0)

        # fuse categorial, affordance, attribute and object context embeddings
        inp_feats = torch.cat((obj_embed2, obj_aff_embed, obj_att_embed, obj_feats), 1)
        ...
        ls_transposed = torch.tensor(ls_transposed, dtype=torch.int64, device='cpu')
    ...
    def obj_ctx(self, obj_feats, obj_dists, im_inds, obj_labels=None, obj_aff_labels=None, obj_att_labels=None,
        ...
        if self.mode != 'predcls':
            ...
            aff_dists, aff_preds = self.aff_decoder_rnn(
                decoder_inp,
                labels=obj_aff_labels[perm] if obj_aff_labels is not None else None,
                boxes_for_nms=None,
            )
            aff_dists = aff_dists[inv_perm]
            aff_preds = aff_preds[inv_perm]

            att_dists, att_preds = self.att_decoder_rnn(
                decoder_inp,
                labels=obj_att_labels[perm] if obj_att_labels is not None else None,
                boxes_for_nms=None,
            )
            att_dists = att_dists[inv_perm]
            att_preds = att_preds[inv_perm]
        else:
            assert obj_labels is not None
            obj_preds = obj_labels
            obj_dists = Variable(to_onehot(obj_preds.data, self.num_classes))

            aff_preds = obj_aff_labels
            aff_dists = Variable(to_multihot(aff_preds.data))

            att_preds = obj_att_labels
            att_dists = Variable(to_multihot(att_preds.data))

        encoder_rep = encoder_rep[inv_perm]

        return obj_dists, obj_preds, aff_dists, aff_preds, att_dists, att_preds, encoder_rep

    def forward(self, obj_fmaps, obj_logits, im_inds, obj_labels=None, obj_aff_labels=None, obj_att_labels=None,
                box_priors=None, boxes_per_cls=None):
        """
        ...
        :return:
        """
        # turn the probability distribution to one-shot and and then extract the embedding
        obj_embed = F.softmax(obj_logits, dim=1) @ self.obj_cate_embed.weight
        # center size -> turn it to cx, cy, w, h
        # pos embedding maps 4x1 to 128x1
        pos_embed = self.pos_embed(Variable(center_size(box_priors)))
        # concatenate all embeddings
        obj_pre_rep = torch.cat((obj_fmaps, obj_embed, pos_embed), 1)

        if self.nl_obj > 0:
            obj_dists2, obj_preds, obj_aff_dist, obj_aff_preds, obj_att_dist, obj_att_preds, obj_ctx = self.obj_ctx(
                obj_pre_rep,
                obj_logits,
                im_inds,
                obj_labels,
        ...
        if self.nl_edge > 0:
            edge_ctx = self.edge_ctx(
                ...
                obj_aff_dists=obj_aff_dist,
                obj_att_dists=obj_att_dist,
                im_inds=im_inds,
                obj_preds=obj_preds,
                obj_aff_preds=obj_aff_preds,
                obj_att_preds=obj_att_preds,
                box_priors=box_priors,
            )

        return obj_dists2, obj_preds, self.m_act(obj_aff_dist), obj_aff_preds, \
               self.m_act(obj_att_dist), obj_att_preds, edge_ctx

        # return obj_dists2, obj_preds, obj_aff_dist, obj_aff_preds, \
        #        obj_att_dist, obj_att_preds, edge_ctx


class RTRelModel(nn.Module):
    """
    Scene Graph Parsing for Visual Entity and their Relationships in Robotic Manipulation
    """
    def __init__(self, classes, aff_classes, att_classes, rel_classes, mode='sgdet', num_gpus=1,
        ...
        :param aff_classes: Affordance classes
        :param att_classes: Attribute classes
        ...
        :param obj_dim:
        """
        super(RTRelModel, self).__init__()
        # class for object category, affordance, attribute
        self.classes = classes
        self.aff_classes = aff_classes
        self.att_classes = att_classes
        self.rel_classes = rel_classes
        ...
        self.detector = GTObjectDetector(
            classes=classes,
        ...
        self.context = LinearizedContext(self.classes, self.aff_classes, self.att_classes, self.rel_classes,
                                         mode=self.mode, embed_dim=self.embed_dim, hidden_dim=self.hidden_dim,
    ...
    @property
    def num_aff_classes(self):
        return len(self.aff_classes)-1

    @property
    def num_att_classes(self):
        return len(self.att_classes)-1

    @property
    def num_rels(self):
        return len(self.rel_classes)-1
    ...
    def forward(self, x, im_sizes, image_offset,
                gt_boxes=None, gt_classes=None, gt_aff_classes=None, gt_att_classes=None,
                gt_rels=None, proposals=None, train_anchor_inds=None, return_fmap=False):
        """
        ...
        """
        result = self.detector(x, im_sizes, image_offset, gt_boxes, gt_classes, gt_aff_classes, gt_att_classes,
        ...
        result.rm_obj_dists, result.obj_preds, result.rm_obj_aff_dists, result.rm_obj_aff_labels, \
        result.rm_obj_att_dists, result.rm_obj_att_labels, edge_ctx = self.context(
            result.obj_fmap,
            result.rm_obj_dists.detach(),
            im_inds, result.rm_obj_labels if self.training or self.mode == 'predcls' else None,
            result.rm_obj_aff_labels,
            result.rm_obj_att_labels,
        ...
        result.rel_dists = F.sigmoid(self.rel_compress(prod_rep))
        ...
        '''
        ### Sum
        # twod_indx_aff = (result.rm_obj_aff_labels.data == 1).nonzero(as_tuple=False)
        # result.obj_aff_scores = torch.zeros_like(result.obj_scores)
        # for index, label in twod_indx_aff:
        #     result.obj_aff_scores[index] += result.rm_obj_aff_dists[index, label]
        #
        # twod_indx_att = (result.rm_obj_att_labels.data == 1).nonzero(as_tuple=False)
        # result.obj_att_scores = torch.zeros_like(result.obj_scores)
        # for index, label in twod_indx_att:
        #     result.obj_att_scores[index] += result.rm_obj_att_dists[index, label]

        ### Multiplication
        twod_indx_aff = (result.rm_obj_aff_labels.data == 1).nonzero(as_tuple=False)
        result.obj_aff_scores = torch.ones_like(result.obj_scores)
        for index, label in twod_indx_aff:
            result.obj_aff_scores[index] *= result.rm_obj_aff_dists[index, label]

        twod_indx_att = (result.rm_obj_att_labels.data == 1).nonzero(as_tuple=False)
        result.obj_att_scores = torch.ones_like(result.obj_scores)
        for index, label in twod_indx_att:
            result.obj_att_scores[index] *= result.rm_obj_att_dists[index, label]
        ...
        # rel_rep = (result.rel_dists > 0.5).float()
        return filter_dets_gt(bboxes, result.obj_scores, result.obj_preds,
                              result.obj_aff_scores, result.rm_obj_aff_labels,
                              result.obj_att_scores, result.rm_obj_att_labels,
                              rel_inds[:, 1:], result.rel_dists)
