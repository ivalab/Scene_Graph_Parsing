...
from lib.fpn.proposal_assignments.proposal_assignments_gtbox import proposal_assignments_gtbox, proposal_assignments_gtbox_3labels
from lib.fpn.proposal_assignments.proposal_assignments_det import proposal_assignments_det, proposal_assignments_det_3labels
...
class Result(object):
    ...
    def __init__(self, od_obj_dists=None, rm_obj_dists=None, rm_obj_aff_dists=None, rm_obj_att_dists=None,
                 ...
                 boxes_assigned=None, boxes_all=None, od_obj_labels=None,
                 rm_obj_labels=None, rm_obj_aff_labels=None, rm_obj_att_labels=None,
...
class GTObjectDetector(nn.Module):
    """
    ...
    def __init__(self, classes, aff_classes, att_classes, mode='rpntrain', num_gpus=1, nms_filter_duplicates=True,
        ...
        """
        super(GTObjectDetector, self).__init__()
        ...
        if not self.use_resnet:
            ...
            # exclusively for resnet50, 101, 152
    ...
    def num_aff_classes(self):
        return len(self.aff_classes)

    @property
    def num_att_classes(self):
        return len(self.att_classes)
    ...
    def rpn_boxes(self, fmap, im_sizes, image_offset, gt_boxes=None, gt_classes=None,
                  gt_aff_classes=None, gt_att_classes=None, gt_rels=None,
        ...
        if self.training:
            ...
            if self.mode == 'refinerels':
                ...
                all_rois, labels, aff_labels, att_labels, bbox_targets = proposal_assignments_det_3labels(
        ...
        return all_rois, labels, aff_labels, att_labels, bbox_targets, rpn_scores, rpn_box_deltas, rel_labels

    def gt_boxes(self, fmap, im_sizes, image_offset, gt_boxes=None, gt_classes=None,
                 gt_aff_classes=None, gt_att_classes=None, gt_rels=None,
                 train_anchor_inds=None, proposals=None):
        """
        Gets GT boxes!
        ...
        if gt_rels is not None and self.training:
            rois, labels, aff_labels, att_labels, rel_labels = proposal_assignments_gtbox_3labels(
                rois.data, gt_boxes.data, gt_classes.data, gt_aff_classes.data, gt_att_classes.data,
                gt_rels.data, image_offset,
                fg_thresh=0.5)
        else:
            labels = gt_classes[:, 1]
            aff_labels = gt_aff_classes[:, 1:]
            att_labels = gt_att_classes[:, 1:]
            rel_labels = None

        return rois, labels, aff_labels, att_labels, None, None, None, rel_labels
    ...
    def forward(self, x, im_sizes, image_offset,
        ...
        # Get boxes from RPN
        rois, obj_labels, obj_aff_labels, obj_att_labels, bbox_targets, rpn_scores, rpn_box_deltas, rel_labels = \
            self.get_boxes(fmap, im_sizes, image_offset, gt_boxes, gt_classes, gt_aff_classes, gt_att_classes,
                           gt_rels, train_anchor_inds, proposals=proposals)
        ...
        if (not self.training and not self.mode == 'gtbox') or self.mode in ('proposals', 'refinerels'):
            ...
            if self.training and not self.mode == 'gtbox':
                ...
                rm_obj_aff_labels = gt_aff_classes[:, 1][argmax_overlaps]
                rm_obj_aff_labels[max_overlaps < 0.5] = 0

                rm_obj_att_labels = gt_att_classes[:, 1][argmax_overlaps]
                rm_obj_att_labels[max_overlaps < 0.5] = 0
            else:
                rm_obj_labels = None
        else:
            ...
            rm_obj_aff_labels = obj_aff_labels
            rm_obj_att_labels = obj_att_labels
        ...
        return Result(
            ...
            rm_obj_aff_labels=rm_obj_aff_labels,
            rm_obj_att_labels=rm_obj_att_labels,
