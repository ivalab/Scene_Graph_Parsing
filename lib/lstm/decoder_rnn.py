...
class DecoderRNN(torch.nn.Module):
    ...
    def forward(self,  # pylint: disable=arguments-differ
        ...
        sequence_tensor, batch_lengths = inputs.data, inputs.batch_sizes
        ...
        for i, l_batch in enumerate(batch_lengths):
            ...
            if self.training:
                ...
                RX version
                '''
                # labels_to_embed = pred_dist[:, :].max(1)[1]
                # out_commitments.append(labels_to_embed)
                # previous_embed = self.obj_embed(labels_to_embed)
            else:
                assert l_batch == 1
...
class MultiLabelDecoderRNN(torch.nn.Module):
    def __init__(self, classes, embed_dim, inputs_dim, hidden_dim, recurrent_dropout_probability=0.2,
                 use_highway=True, use_input_projection_bias=True):
        """
        Initializes the RNN
        ...
        :param unk: unknown token (not used)
        """
        super(MultiLabelDecoderRNN, self).__init__()
        ...
        self.out = nn.Linear(self.hidden_size, len(self.classes)-1)
        self.reset_parameters()
    ...
    def forward(self,  # pylint: disable=arguments-differ
        ...
        for i, l_batch in enumerate(batch_lengths):
            ...
            if self.training:
                ...
                # labels_to_embed = labels[start_ind:end_ind].clone()
                # # Whenever labels are 0 set input to be our max prediction
                # nonzero_pred = pred_dist[:, :].max(1)[1] + 1
                # is_bg = (labels_to_embed.data == 0).nonzero()
                # if is_bg.dim() > 0:
                #     labels_to_embed[is_bg.squeeze(1)] = nonzero_pred[is_bg.squeeze(1)]
                # out_commitments.append(labels_to_embed)
                ...
                # out_dist_sample = F.sigmoid(pred_dist, dim=1)
                # labels_to_embed = (out_dist_sample > 0.5).float()

                out_commitments.append(labels_to_embed)
                previous_embed = None
                for pred_i in labels_to_embed:
                    bbx_embed = None
                    for label_idx, pred_value in enumerate(pred_i):
                        if pred_value == 1:
                            if bbx_embed is None:
                                bbx_embed = self.obj_embed(torch.tensor(label_idx + 2, dtype=torch.int64, device='cuda'))
                            else:
                                bbx_embed += self.obj_embed(torch.tensor(label_idx + 2, dtype=torch.int64, device='cuda'))

                    if bbx_embed is None:
                        bbx_embed = self.obj_embed(torch.tensor(1, dtype=torch.int64, device='cuda'))
                    bbx_embed = bbx_embed.unsqueeze(0)

                    if previous_embed is None:
                        previous_embed = bbx_embed
                    else:
                        previous_embed = torch.cat((previous_embed, bbx_embed), 0)
            else:
                '''
                RX
                '''
                assert l_batch == 1
                out_dist_sample = F.sigmoid(pred_dist)

                best_ind = (out_dist_sample > 0.5).float()

                out_commitments.append(best_ind)

                previous_embed = None
                for pred_i in best_ind:
                    bbx_embed = None
                    for label_idx, pred_value in enumerate(pred_i):
                        if pred_value == 1:
                            if bbx_embed is None:
